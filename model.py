...
class ModelsBenchmark(collections.UserList):
    ...
    def compute_scores(self, train_data, test_data):
        return [(Model(model).compute_scores(train_data, test_data)[0], model) for model in self]

    def get_sorted_scores(self, train_data, test_data):
        scores = self.compute_scores(train_data, test_data)
